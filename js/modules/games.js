const GamesModule = {
  ...
  handle(text) {
    ...
    // Крестики-нолики
    if (/крестики|нолики|tictactoe|х[о0]/.test(t) && !this.state.active) return this.startTicTacToe();

    // Правда или действие
    if (/правда.*действ|действ.*правд/.test(t)) return this.startTruth();

    // Кто я
    if (/^кто я|кто я\?|угадай кто/.test(t)) return this.startWhoAmI();
  ...
  // === КТО Я? ===
  whoamiList: [
    "Пушкин", "Наполеон", "Эйнштейн", "Моцарт", "Пикассо",
    "Гагарин", "Ньютон", "Леонардо да Винчи", "Шекспир", "Клеопатра",
  ...
  playWhoAmI(text) {
    ...
    if (this.state.whoamiTries > 10) {
      const s = this.state.secret;
      this.state.active = null;
      return { text: `Хватит попыток! Это был(а) ${s}. Сыграем ещё? Напиши «кто я».` };
    }

    // Простые подсказки по категориям
  ...
  isPerson(s) {
    return /пушкин|наполеон|эйнштейн|моцарт|пикассо|гагарин|ньютон|да винчи|шекспир|клеопатра/i.test(s);
  },
  isAnime(s) {
    return /чебур|гарри|наруто|пикачу|сейлор/i.test(s);
  }
};