...
// Ловит: имя, возраст, класс, интересы, дату рождения
// ============================================================
...
const PersonalModule = {
  ...
  async handle(text) {
    ...
    // ============================================================
    // СОХРАНИТЬ ВОЗРАСТ И КЛАСС ВМЕСТЕ
    // «Мне 13, я в 7 классе»
    // ============================================================
    if (/мне\s+\d+.*(\d{1,2})\s*класс/i.test(text)) {
      const combinedMatch = text.match(/мне\s+(\d{1,3}).*?(\d{1,2})\s*класс/i);
      if (combinedMatch) {
        const age = parseInt(combinedMatch[1]);
        const grade = parseInt(combinedMatch[2]);
        if (age > 0 && age < 120) await MemoryDB.set("age", age);
        if (grade >= 1 && grade <= 11) await MemoryDB.set("grade", grade);
        return { text: `Запомнила! Тебе ${age} ${this.plural(age, "год", "года", "лет")}, ты в ${grade} классе 💖` };
      }
    }
    ...
    // ДАТА РОЖДЕНИЯ
    // ============================================================
    if (/(моё день рождени|мое день рождени|когда.*день рождени|мой др)/.test(t)) {
      const bday = await MemoryDB.get("birthday");
      if (bday) {
        return { text: `Твой день рождения — ${bday} 🎂` };
      }
      return { text: "Ты не говорил дату рождения 🤔 Напиши: «Мой день рождения 15 марта»" };
    }

    // Сохранить день рождения
    const bdayMatch = text.match(/(?:мой день рождени|мое день рождени|день рождени)\s*[-—:]?\s*(\d{1,2})[.\s\/\-]?(\d{1,2})?/i);
    if (bdayMatch && /день рожден/i.test(text)) {
      const day = bdayMatch[1];
      const month = bdayMatch[2] || "";
      const bday = month ? `${day}.${month}` : day;
      await MemoryDB.set("birthday", bday);
      return { text: `Запомнила! Твой день рождения — ${bday} 🎂 Поздравлю обязательно!` };
    }
    ...
    // ЗАБУДЬ ПРО МЕНЯ
    // ============================================================
    if (/(забудь.*меня|удали.*данные|сотри.*память|забудь.*имя|очисти.*данные|забудь.*класс|забудь.*возраст)/.test(t)) {
      await MemoryDB.delete("name");
      await MemoryDB.delete("age");
      await MemoryDB.delete("grade");
      await MemoryDB.delete("favorite_topic");
      await MemoryDB.delete("birthday");
  ...
  async getProfile() {
    ...
    out += `🎂 День рождения: ${bday || "неизвестно"}\n`;
